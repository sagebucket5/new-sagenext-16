const BASE_URL = "https://snb.thesagenext.com/blog/index.php/wp-json/wp/v2";
...
export async function getCategorie(cat_id) {
  const res = await fetch(BASE_URL + `/posts?_embed&per_page=6&categories=${cat_id}`);
  const categories = res.json();
  return categories;
}
//Get all Categories
export async function getAllCategories() {
  const catRes = await fetch(BASE_URL + "/categories?per_page=20");
...
export async function getAllPost(page) {
  ...
  try {
    const postsRes = await fetch(BASE_URL +  `/index.php/wp-json/wp/v2/posts?_embed&page=${page}&per_page=20`);
